...
from test_plotting import fix_sharey_labels, add_unified_legend_below, add_grouped_legends_below
...
def datadecide_instance():
    ...
    if not data_dir.exists():
        pytest.skip(f"Data directory {data_dir} does not exist. Please run data pipeline first.")
...
def extract_line_properties(line):
    ...
    return {
        ...
        'xdata_range': [float(np.min(xdata)), float(np.max(xdata))] if len(xdata) > 0 else [0, 0],
        'ydata_range': [float(np.min(ydata)), float(np.max(ydata))] if len(ydata) > 0 else [0, 0],
...
def extract_legend_properties(legend):
    ...
    for handle in handles:
        if hasattr(handle, 'get_color'):  # Line2D or similar
            handle_props.append({
                ...
                'linestyle': handle.get_linestyle() if hasattr(handle, 'get_linestyle') else None,
    ...
    return {
        ...
        'bbox_to_anchor': legend.get_bbox_to_anchor().bounds if legend.get_bbox_to_anchor() else None,
...
def extract_complete_plot_state(fig, fm=None):
    ...
    state = {
        'figure': extract_figure_properties(fig),
        'axes': [],
        'figure_legends': []
    }
...
class TestConfig1:
    ...
    def test_config1_visual_properties(self, test_dataframe, test_params, test_data):
        ...
        for ylim in y_limits:
            assert abs(ylim[0] - first_ylim[0]) < 1e-10  # Allow floating point precision
        ...
        for ax_state in state['axes']:
            ...
            assert len(set(line_colors)) == len(ax_state['lines'])  # All lines have different colors
            
        # Verify legends exist (either figure-level or axis-level) 
        total_legend_count = len(state['figure_legends'])
        axis_legend_count = sum(1 for ax_state in state['axes'] if ax_state['legend'] is not None)
        assert total_legend_count + axis_legend_count >= 1  # Should have at least some legend content
        ...
        if total_legend_count > 0:
            ...
            if legend_props and legend_props['texts_ordered']:
                ...
                for param in expected_param_order:
                    assert any(param in text for text in legend_texts), f"Param {param} not found in legend"
        ...
        for ax_idx, ax_state in enumerate(state['axes']):
            for line_idx, line_props in enumerate(ax_state['lines']):
                ...
                assert line_props['finite_data_count'] > 0, f"Axis {ax_idx}, Line {line_idx} has no finite data points"
                assert len(line_props['xdata_values']) == line_props['finite_data_count']
                assert len(line_props['ydata_values']) == line_props['finite_data_count']
                # Verify data points are reasonable (positive tokens, reasonable perplexity)
                if len(line_props['xdata_values']) > 0:
                    assert all(x > 0 for x in line_props['xdata_values']), "X values (tokens) should be positive"
                    assert all(y > 0 for y in line_props['ydata_values']), "Y values (perplexity) should be positive"
...
class TestConfig2:
    ...
    def test_config2_visual_properties(self, test_dataframe, test_params, test_data):
        ...
        for data_recipe in expected_data_order:
            ...
            assert any(data_recipe in label for label in line_labels), f"Data recipe {data_recipe} not found in line labels"
            ...
            if legend_props and legend_props['texts_ordered']:
                ...
                for ax_state in state['axes']:
                    for line in ax_state['lines']:
                        if line['label'] and not line['label'].startswith('_'):  # Skip internal matplotlib labels
                            unique_line_labels.add(line['label'])
                
                for line_label in unique_line_labels:
                    assert any(line_label == legend_text for legend_text in legend_texts), f"Line label '{line_label}' not found in legend"
...
class TestConfig3:
    ...
    def test_config3_visual_properties(self, test_dataframe, test_params, test_data):
        ...
        expected = {
            'figure': {
                'figsize': [25.0, 4.0],  # 5 data * 5 width, default height (different from Config 1)
                'num_axes': 5,
            },
            'axes_count': 5,
            'lines_per_axis': 4,
...
class TestConfig4:
    ...
    def test_config4_visual_properties(self, test_dataframe):
        ...
        expected = {
            ...
            'axes_count': 2,
            'expected_line_count_per_axis': len(config4_data) * len(config4_params),  # 4 * 3 = 12
        }
        ...
        if total_legend_count >= 2:
            ...
            legend_titles = [legend['title'] for legend in state['figure_legends'] if legend]
            legend_titles = [title for title in legend_titles if title]  # Remove None values
            ...
            for expected_title in expected_titles:
                assert any(expected_title in str(title) for title in legend_titles), f"Legend title '{expected_title}' not found"
        ...
        for ax_state in state['axes']:
            ...
            assert len(unique_styles) > 1, "Config 4 should have multiple line styles for different param sizes"
...
class TestConfig5:
    ...
    def test_config5_visual_properties(self, test_dataframe, test_params, test_data):
        ...
        single_data = [test_data[0]] if test_data else [sorted(test_dataframe["data"].unique())[0]]
    ...
    def test_config6_visual_properties(self, test_dataframe, test_params, test_data):
        ...
        expected = {
            'figure': {
                ...
                'num_axes': len(config6_metrics) * len(test_data),  # 2 metrics * 5 data = 10 axes
            },
        ...
        unique_y_positions = set(round(pos[1], 3) for pos in positions)  # Y positions (rows)
        unique_x_positions = set(round(pos[0], 3) for pos in positions)  # X positions (cols)
    ...
    def test_config7_visual_properties(self, test_dataframe):
        ...
        expected = {
            'figure': {
                ...
                'num_axes': len(config7_metrics) * len(config7_params),  # 2 metrics * 5 params = 10 axes
            },
        ...
        for data_recipe in config7_data:
            assert any(data_recipe in label for label in line_labels), f"Expected data recipe {data_recipe} not found"
        
        # Verify that 50/50 recipe is NOT present
        excluded_recipe = "DCLM-Baseline 50% / Dolma 50%"
        assert not any(excluded_recipe in label for label in line_labels), f"Excluded recipe {excluded_recipe} should not be present in Config 7"
        
        # Verify 4-color progression (not 5-color) due to reduced data set
        for ax_state in state['axes']:
            line_colors = [line['color'] for line in ax_state['lines']]
            unique_colors = set(line_colors)
            assert len(unique_colors) == 4, f"Config 7 should have exactly 4 colors, got {len(unique_colors)}"
