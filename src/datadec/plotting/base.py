...
class BasePlotBuilder:
        ...
        if dropna and key_columns:
            ...
            if available:
                ...
                if self.config.get("verbose"):
                    print(
                        f"After NaN filtering: {self.plot_df.shape} (removed {initial_shape[0] - self.plot_df.shape[0]} rows)"
                    )
        return self
        ...
        if params_list and "line_col" in self.config and "subplot_col" in self.config:
            if "params" in self.config["line_col"]:
                self.plot_df = self.plot_df[
                    self.plot_df[self.config["line_col"]].isin(params_list)
                ]
            elif "params" in self.config["subplot_col"]:
                self.plot_df = self.plot_df[
                    self.plot_df[self.config["subplot_col"]].isin(params_list)
                ]
        return self
    ...
    def filter_data(self, data_list: Optional[List[str]]):
        ...
        if data_list and "line_col" in self.config and "subplot_col" in self.config:
            if "data" in self.config["line_col"]:
                self.plot_df = self.plot_df[
                    self.plot_df[self.config["line_col"]].isin(data_list)
                ]
            elif "data" in self.config["subplot_col"]:
                self.plot_df = self.plot_df[
                    self.plot_df[self.config["subplot_col"]].isin(data_list)
                ]
        return self
    ...
    def validate_data(self, verbose: bool = False):
        ...
        if verbose:
            ...
            if "data" in self.plot_df.columns:
                ...
                print(
                    f"Unique data: {data_values[:10]}..."
                    if len(data_values) > 10
                    else f"Unique data: {data_values}"
                )
        return self

    def _calculate_layout(
        self, subplot_col: str
    ) -> Tuple[List, int, int, Tuple[int, int], dict]:
        """
    ...
    def _generate_sequential_colors(
        self,
        hue_values: List[str],
        colormap: str = "plasma",
        color_range_min: float = 0.1,
        color_range_max: float = 1.0,
        two_color_start: str = None,
        two_color_end: str = None,
        ...
        if multi_color_sequence is not None and len(multi_color_sequence) >= 2:
            ...
            cmap = self._create_multi_color_colormap(
                multi_color_sequence, "custom_multi_color"
            )
        elif two_color_start is not None and two_color_end is not None:
            # Create custom two-color colormap
            cmap = self._create_two_color_colormap(
                two_color_start, two_color_end, "custom_two_color"
            )
        else:
    ...
    def _create_two_color_colormap(
        self, start_color: str, end_color: str, name: str = "custom"
    ...
    def _create_multi_color_colormap(
        self, colors: List[str], name: str = "custom_multi"
    ) -> LinearSegmentedColormap:
        """
    ...
    def _generate_sequential_linestyles(
        self, style_values: List[str], linestyle_sequence: List[str] = None
    ) -> Dict[str, str]:
        """
    ...
    def _sort_data_for_consistency(
        self, subset: pd.DataFrame, sort_col: str
    ) -> pd.DataFrame:
        """
    ...
    @classmethod
    def from_datadecide(
        cls, dd, dataframe_name: str = "mean_eval", verbose: bool = False
    ):
        """
