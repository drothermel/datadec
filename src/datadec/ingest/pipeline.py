...
import datadec.ingest.utils as du
...
def build_prefixed_column_aliases(
    ...
    for col, col_type in zip(columns, column_types, strict=True):
        ...
        dtype = col_type.upper() if isinstance(col_type, str) else str(col_type).upper()
...
def _build_select_clause(
    preds_rel: duckdb.DuckDBPyRelation,
    recs_rel: duckdb.DuckDBPyRelation,
    reqs_rel: duckdb.DuckDBPyRelation,
    met_rel: duckdb.DuckDBPyRelation,
) -> str:
    prefix_map = {
...
def export_eval_dir_to_parquet(
    ...
    *,
    ...
    doc_filter = "" if doc_id_value is None else f" WHERE doc_id = {doc_id_value}"
    req_filter = "" if doc_id_value is None else f" WHERE doc_id = {doc_id_value}"

    null_idx = "CAST(NULL AS BIGINT) AS idx"
    select_sql = f"""
        WITH doc_keys AS (
            SELECT DISTINCT file_prefix, doc_id, {null_idx} FROM preds{doc_filter}
            UNION
            SELECT DISTINCT file_prefix, doc_id, {null_idx} FROM recs{doc_filter}
            UNION
...
def clean_cached_results(
    ...
    *,
    ...
    if has_config:
        ...
        def _normalize_config(value: Any) -> str | None:
            if value is None:
                return None
    ...
    drop_candidates = {
        col for col in columns if col in drop_set or col.endswith(("_path", "_dir"))
    }
    ...
    if has_config:
        ...
        copy_sql = (
            f"COPY (SELECT DISTINCT {projection} FROM cached_runs) "
            f"TO '{escaped_out}' (FORMAT PARQUET)"
        )
    conn.sql(copy_sql)
