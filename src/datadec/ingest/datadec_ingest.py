...
from datadec.ingest.paths import Paths
...
def _resolve_defaults(
    metrics_dir: Path | None, cache_dir: Path | None
) -> tuple[Path, Path]:
    defaults = Paths()
...
def cache(
    ...
    results_dir: list[Path] | None = typer.Option(
        None,
    ...
    if results_dir:
        ...
        entries = [
            entry for entry in entries if Path(entry["results_dir"]).resolve() in wanted
        ]
