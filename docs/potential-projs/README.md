...
| [Text-latent code autoencoder](text-latent-code-autoencoder.md) (`TLC`) | Frozen-LLM encoder/decoder with a text latent and prompts as the only learned object. Three layers: a measurement suite (TLC-0: 𝒱-information retention vs. implementation leakage against a declared extractor family) shared by two separate projects — the compression project (TLC-1–3: can lossy functional compression + lossless coding beat the strongest fixed-prior lossless baselines of code) and the representation program (TLC-opt-*: what representations can be induced, how portable and controllable they are, whether function and style factor) | none (API inference only) | API | Workshop from the compression result; main-conference against LLM arithmetic coding; thesis-scale representation program |
...
| [DataDecide data card](datadecide-data-card.md) (`DCARD`) | What do DataDecide's published artifacts actually support: a provenance ledger of self-description vs. ground truth, a claim-by-claim reproduction on the cleaned tables, a coverage ledger, and the reconstructions that make dynamics analyses valid — released as a validated dataset | none | T0 (T1 for own-mixture CE) | Resource / D&B paper; every DataDecide project cites it |
