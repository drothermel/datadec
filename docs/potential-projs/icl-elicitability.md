...
yet written. Full inventory: `related-work/icl-elicitability.md`.**

The load-bearing items on record, with the role each plays:

- **Chan et al. 2022, *Data distributional properties drive emergent in-context learning
  in transformers*** — the closest neighbor and the model for ICL-opt-1's tiny tier:
  pretraining data properties decide whether ICL emerges at all, often at similar training
  loss. **Raventós et al.** (task-diversity threshold for regression ICL) is its
  linear-model counterpart. (`../topics/reference/icl-literature.md`)
- **Olsson et al., *In-context Learning and Induction Heads*** — the metric precedent for
  protocol statistics 1–2: the original ICL score as an early-vs-late token loss
  difference, plus per-checkpoint prefix-matching/copying scores. Also the sharp
  phase-transition claim ICL-opt-3's prediction rests on.
- ***What ICL "Learns" In-Context: Disentangling Task Recognition and Task Learning*** —
  supplies statistic 3's shuffled-label and format-only controls.
- **Hendel et al. (task vectors) and Todd et al. (function vectors)**, with Danielle-supplied
  **Dong et al. 2506.09048** (high-rank mappings break extracted vectors — a task-selection
  constraint) and **Yang et al. 2509.24169** (learned rather than extracted vectors —
  ICL-opt-6). These define statistic 4. (`../topics/reference/task-vectors.md`)
- **The ICL-as-gradient-descent arc** (von Oswald; Akyürek; Dai; Ahn) with its two
  contradictions — ***ICL and Gradient Descent Revisited*** (untrained models score
  comparably) and ***The Initialization Determines Whether ICL Is Gradient Descent*** —
  is why statistic 5 runs only with the untrained-model control.
- ***Eliciting Fine-Tuned Transformer Capabilities via Inference-Time Techniques*** and
  **Bornschein, Lyle, Pascanu et al., *Fine-Tuned In-Context Learners for Efficient
  Adaptation*** — the "two access routes to the same capabilities" premise behind ICL-4;
  both agent-sourced and unverified.
- **Lester, Al-Rfou & Constant 2021 (prompt tuning)** — the contrast case and known
  headwind: elicitation is weak at small scale, recorded "from memory; verify."
- **Juneja et al. (ICLR 2023)** — the comparability constraint: mechanism-level metrics may
  not be comparable across basins, hence barrier-conditioned reporting in ICL-2.
- **Rothermel et al. 2021 (arXiv 2107.12460)** — Danielle's own framing anchor; §4 frames
  the project as quantifying the gap that paper found, and the frozen-body audit's G6
  (ICL-opt-5) records that no paper on file reframes it as an elicitation ceiling.
- ***In-Context Learning with Long-Context Models*** (NAACL 2025) and the many-shot scaling
  leads — the only usable ICL-opt-7 precedent, and the topic file marks the surrounding
  author attributions as apparently fabricated: titles and one ID as search leads only.

Full enumeration with sources in `related-work/icl-elicitability.md`. Main accumulators:
`../topics/reference/icl-literature.md`, `../topics/reference/task-vectors.md`,
`../topics/reference/pretraining-to-posttraining.md`, `../topics/reference/plasticity.md`,
`../topics/reference/critical-periods.md`, `../topics/reference/landscape-literature.md`,
`../topics/staging/frozen-body-transfer-audit.md`,
`../topics/staging/clean-code-preference-icl.md`, `../research-hypothesis.md`.
