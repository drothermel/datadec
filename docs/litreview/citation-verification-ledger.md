...
| 2304.12512 | Gilbert et al. 2023 — Semantic compression with LLMs (submitted paper's bibliography) | `text-latent-code-autoencoder` §4 (submission record) | Danielle-supplied | TLC | |
...
| (no ID) | SAPS 2018 — "Ain't Nobody Got Time For Coding": structure-aware program synthesis from NL, tree-to-tree autoencoder (SciSpace summary table, 1.5/6; no ID in source) | `nl-bottleneck-prior-art` (table-layer entry) | agent-supplied (NBLM/SciSpace) | TLC | |
