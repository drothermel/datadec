...
| (no ID) | ShortenDoc — docstring compression on HumanEval/EvoEval; no identifier given in the response | `code-benchmarks-landscape` (HumanEval afterlife) | agent-supplied | TLC (gate 1) | NBLM AI4SE notebook (2026-08-24) adds full method detail + the method-name-quality dependency; see code-compression-literature entry |
...
| 2602.03712 | SWE-Refactor | `code-benchmarks-landscape` (2026-08-24 Perplexity entry) | agent-supplied (Perplexity, April convo) | — | NBLM refactoring notebook (2026-08-24) adds full detail (1,099 Java refactorings; AST verification stack; OpenReview follow-up baselines) |
...
| (no ID) | Eureka (Ma et al. 2023, NVIDIA/Penn) — LLM optimizes reward functions, RL inner loop; 83% of 29 envs | `prompt-optimization-landscape` (2026-08-24 LLM-as-optimizer entry) | agent-supplied (Perplexity, LLM-as-optimizer convo) | TLC-opt / minigrid staging | |
| 2506.13131 | AlphaEvolve (2025) — FunSearch follow-up, RL fine-tuned mutation operator; agentic coding for scientific/algorithmic discovery | `prompt-optimization-landscape` (2026-08-24 LLM-as-optimizer entry) | agent-supplied (Perplexity, LLM-as-optimizer convo) | TLC-opt | ID from the NBLM LLM-optimization notebook (agent-supplied) |
| (no ID) | LLAMBO (Liu et al. 2024) — LLM replaces surrogate + acquisition in Bayesian optimization | `prompt-optimization-landscape` (2026-08-24 LLM-as-optimizer entry) | agent-supplied (Perplexity, LLM-as-optimizer convo) | TLC-opt | |
| (no ID) | ReEvo (Ye et al. 2024) — reflective evolution on NP-hard combinatorial problems | `prompt-optimization-landscape` (2026-08-24 LLM-as-optimizer entry) | agent-supplied (Perplexity, LLM-as-optimizer convo) | TLC-opt | |
| (no ID) | LLM-SR (Shojaee et al. 2024) — equation-skeleton symbolic regression via LLM proposals | `prompt-optimization-landscape` (2026-08-24 LLM-as-optimizer entry) | agent-supplied (Perplexity, LLM-as-optimizer convo) | TLC-opt | |
| (no ID) | DICL / zero-shot model-based RL (Benechehab et al. 2024) — LLM in the transition-model slot | `prompt-optimization-landscape` (2026-08-24 LLM-as-optimizer entry) | agent-supplied (Perplexity, LLM-as-optimizer convo) | minigrid staging | |
| (no ID) | Matryoshka — small white-box LLM guiding large black-box LLM; named closest to the dual-LLM autoencoder framing | `prompt-optimization-landscape` (2026-08-24 LLM-as-optimizer entry) | agent-supplied (Perplexity, LLM-as-optimizer convo) | TLC | |
| (no ID) | ACING — actor-critic instruction optimization for black-box LLMs | `prompt-optimization-landscape` (2026-08-24 LLM-as-optimizer entry) | agent-supplied (Perplexity, LLM-as-optimizer convo) | TLC | |
| (no ID) | Sensory-Motor Control with LLMs via Iterative Prompting (2025) — LLM writes control programs; CartPole solved by o1-class | `prompt-optimization-landscape` (2026-08-24 LLM-as-optimizer entry) | agent-supplied (Perplexity, LLM-as-optimizer convo) | minigrid staging | |
| (no ID) | Bosio & Mueller 2024 — interpretable control policies via LLM-guided search (pendulum, ball-in-cup) | `prompt-optimization-landscape` (2026-08-24 LLM-as-optimizer entry) | agent-supplied (Perplexity, LLM-as-optimizer convo) | minigrid staging | |
| (no ID) | Atari-GPT (Waytowich et al. 2024) — negative result: LLMs fail as direct low-level policies | `prompt-optimization-landscape` (2026-08-24 LLM-as-optimizer entry) | agent-supplied (Perplexity, LLM-as-optimizer convo) | minigrid staging | |
| (no ID) | VeLO — general-purpose learned optimizer (82 tasks, 4,000 TPU-months; fails on RL, lags 8B+ LMs); Harrison talk | `plasticity` (2026-08-24 NBLM notebook entry) | agent-supplied (NotebookLM CL notebook; talk-transcript-derived) | reset staging / whetstone context |  |
| (no ID) | STAR — stability-inducing learned-optimizer architectures (Harrison); paired name 'BALO' garbled, possibly VeLO | `plasticity` (2026-08-24 NBLM notebook entry) | agent-supplied (NotebookLM CL notebook; talk-transcript-derived) | whetstone context | garbled transcript name |
| (no ID) | Van Roy CCRL — CL as computationally constrained RL; C log C; informational-vs-inferential error decomposition (CoLLAs 2023 talk) | `plasticity` (2026-08-24 NBLM notebook entry) | agent-supplied (NotebookLM CL notebook; talk-transcript-derived) | nonstationarity pillar |  |
| (no ID) | L2-toward-initial-weights beats Continual Backprop across Van Roy benchmarks | `plasticity` (2026-08-24 NBLM notebook entry) | agent-supplied (NotebookLM CL notebook; talk-transcript-derived) | reset staging | actionable baseline claim |
| (no ID) | Bing Liu — CIL learnable iff OOD detection learnable; WP/TP decomposition (CoLLAs 2023 talk) | `plasticity` (2026-08-24 NBLM notebook entry) | agent-supplied (NotebookLM CL notebook; talk-transcript-derived) | — |  |
| (no ID) | Sokar et al. ICML 2023 (garbled 'Socar') — resetting dormant/dead units in DQN | `plasticity` (2026-08-24 NBLM notebook entry) | agent-supplied (NotebookLM CL notebook; talk-transcript-derived) | reset staging | name inferred from garbled transcript |
| (no ID) | BNSL — Broken Neural Scaling Laws (inflections, double descent) | `plasticity` (2026-08-24 NBLM notebook entry) | agent-supplied (NotebookLM CL notebook; talk-transcript-derived) | loss-curve forecasting |  |
| (no ID) | Head-to-Toe (H2T) probing — all-layer linear probe, <1% FLOPs of fine-tuning (Evci/Ramasesh/Mozer per garbled report) | `plasticity` (2026-08-24 NBLM notebook entry) | agent-supplied (NotebookLM CL notebook; talk-transcript-derived) | frozen-body staging |  |
| (no ID) | URT — universal representation transformer over multiple backbones (Larochelle talk) | `plasticity` (2026-08-24 NBLM notebook entry) | agent-supplied (NotebookLM CL notebook; talk-transcript-derived) | — |  |
| (no ID) | Two-hot categorical target trick — less plasticity loss than L2 regression (Lyle talk) | `plasticity` (2026-08-24 NBLM notebook entry) | agent-supplied (NotebookLM CL notebook; talk-transcript-derived) | reset staging |  |
| (no ID) | CReLU for saturation prevention (Lyle talk) | `plasticity` (2026-08-24 NBLM notebook entry) | agent-supplied (NotebookLM CL notebook; talk-transcript-derived) | reset staging |  |
| (no ID) | Stability gap + EMA-evaluation-model mitigation (CL tutorial part II) | `plasticity` (2026-08-24 NBLM notebook entry) | agent-supplied (NotebookLM CL notebook; talk-transcript-derived) | nonstationarity pillar |  |
| (no ID) | Realistic CL stream benchmarks — CLEAR, Wild-Time, CLOC (39M images), CORe50 (one row) | `plasticity` (2026-08-24 NBLM notebook entry) | agent-supplied (NotebookLM CL notebook; talk-transcript-derived) | nonstationarity pillar |  |
| (no ID) | Continual pretraining with soft gradient masking + LR re-warming (Pile→SlimPajama; Rish/Liu talks) | `plasticity` (2026-08-24 NBLM notebook entry) | agent-supplied (NotebookLM CL notebook; talk-transcript-derived) | schedules/annealing context |  |
| (no ID) | Shrivastava & Tarlow (garbled 'Vastava') — repo-level prompt augmentation for Codex | `plasticity` (2026-08-24 NBLM notebook entry) | agent-supplied (NotebookLM CL notebook; talk-transcript-derived) | TLC (context flank) | name inferred from garbled transcript |
| 2505.15055 | PSN-IRT / Lost in Benchmarks? (Hongli Zhou 2025) — 4PL pseudo-Siamese IRT, benchmark diagnosis | `irt-literature` (2026-08-24 NBLM eval-notebook entry) | agent-supplied (NotebookLM evaluation notebook) | IRT |  |
| 2508.09662 | EffiEval (Yaoning Wang 2025) — Model Utility Index capability-coverage subsets | `irt-literature` (2026-08-24 NBLM eval-notebook entry) | agent-supplied (NotebookLM evaluation notebook) | IRT, TINY |  |
| 2412.06745 | ONEBench (Ghosh 2024) — sample-level pooled benchmarking, Plackett-Luce aggregation | `irt-literature` (2026-08-24 NBLM eval-notebook entry) | agent-supplied (NotebookLM evaluation notebook) | IRT |  |
| 2501.17200 | Federiakin — psychometric leaderboard methodology (FA, g-factor, anchor items, DIF) | `irt-literature` (2026-08-24 NBLM eval-notebook entry) | agent-supplied (NotebookLM evaluation notebook) | IRT |  |
| 2504.09979 | ResampledBench (2025) — farthest-point-sampling subsets, 27 VLMs | `irt-literature` (2026-08-24 NBLM eval-notebook entry) | agent-supplied (NotebookLM evaluation notebook) | IRT, TINY |  |
| 2602.07909 | SparseEval (Taolin Zhang 2026) — sparse-optimization evaluation | `irt-literature` (2026-08-24 NBLM eval-notebook entry) | agent-supplied (NotebookLM evaluation notebook) | IRT, TINY |  |
| (no ID) | Benchmark² (Qi Qian 2025) — CAD/DS/CBRC/BQS benchmark meta-metrics | `irt-literature` (2026-08-24 NBLM eval-notebook entry) | agent-supplied (NotebookLM evaluation notebook) | IRT |  |
| (no ID) | EvaLearn (Shihan Dou, NeurIPS 2025) — sequential learning-capability evaluation | `irt-literature` (2026-08-24 NBLM eval-notebook entry) | agent-supplied (NotebookLM evaluation notebook) | IRT |  |
| (no ID) | Reliable and Efficient Amortized Model-based Evaluation (Sang Truong 2024) — Fisher-info adaptive testing, amortized calibration, difficulty-targeted question generation | `irt-literature` (2026-08-24 NBLM eval-notebook entry) | agent-supplied (NotebookLM evaluation notebook) | IRT |  |
| (no ID) | ADeLe — cognitive-rubric item-demand capability profiles | `irt-literature` (2026-08-24 NBLM eval-notebook entry) | agent-supplied (NotebookLM evaluation notebook) | IRT |  |
| (no ID) | Owen 2024 — How predictable is language model benchmark performance? (sigmoid fits; BBH ~6pp MAE) | `irt-literature` (2026-08-24 NBLM eval-notebook entry) | agent-supplied (NotebookLM evaluation notebook) | TINY, EDP | routed via evaluation-methodology-literature |
| (no ID) | Zeng — pretraining indicators at fixed scale (PPL-CLM worse than chance for SFT selection; PPL-SC/k-shot/LTC) | `irt-literature` (2026-08-24 NBLM eval-notebook entry) | agent-supplied (NotebookLM evaluation notebook) | EDP, elicitation | dual title: 'Can Pre-training Indicators…' vs 'When Scale is Fixed…'; routed via evaluation-methodology-literature |
| (no ID) | RUPBench (Yuqing Wang 2024) — nine perturbation types, Performance Drop Rate | `irt-literature` (2026-08-24 NBLM eval-notebook entry) | agent-supplied (NotebookLM evaluation notebook) | — | routed via evaluation-methodology-literature |
| (no ID) | SciRIFF / SciRIFF-Eval (Wadden) + SciLitLLM — scientific-literature instruction following, structured outputs | `irt-literature` (2026-08-24 NBLM eval-notebook entry) | agent-supplied (NotebookLM evaluation notebook) | structured-output thread | routed via evaluation-methodology-literature |
| (no ID) | LAPS (Wong et al. 2021) — language-grounded library learning; LILO precursor | `nl-bottleneck-prior-art` (2026-08-24 NBLM notebook entry) | agent-supplied (NotebookLM NL-latents notebook) | TLC |  |
| (no ID) | O'Connor & Andreas 2021 — ordering effects / context fragility | `nl-bottleneck-prior-art` (2026-08-24 NBLM notebook entry) | agent-supplied (NotebookLM NL-latents notebook) | TLC |  |
| (no ID) | Self-consistency (Wang et al. 2022) — cited as AutoDoc-verification candidate | `nl-bottleneck-prior-art` (2026-08-24 NBLM notebook entry) | agent-supplied (NotebookLM NL-latents notebook) | TLC |  |
| (no ID) | CompleteP / Completed Hyperparameter Transfer — µP + QK-norm + AdamW-ε; per-module HPs at 50M → 420× transfer | `schedules-and-annealing-literature` (2026-08-24 NBLM entry) | agent-supplied (NotebookLM pretraining notebook) | WSD, TINY | detail fill for the bare name in parametrization-and-hp-transfer |
| (no ID) | Learning Dynamics in Continual Pre-Training — CPT scaling law; loss potential; replay/peak-LR targeting | `schedules-and-annealing-literature` (2026-08-24 NBLM entry) | agent-supplied (NotebookLM pretraining notebook) | WSD, nonstationarity |  |
| (no ID) | Learning Rate Matters: Vanilla LoRA May Suffice — variant gains are LR-tuning artifacts; init sets max Hessian eigenvalue | `schedules-and-annealing-literature` (2026-08-24 NBLM entry) | agent-supplied (NotebookLM pretraining notebook) | elicitation/tuning-equity |  |
| (no ID) | Learning is Forgetting: LLM Training as Lossy Compression — IB at LLM scale, soft-entropy MI on OLMo2; compression-bound proximity predicts downstream | `schedules-and-annealing-literature` (2026-08-24 NBLM entry) | agent-supplied (NotebookLM pretraining notebook) | TRAJ |  |
| (no ID) | Mid-Training of Large Language Models: A Survey — first mid-training taxonomy (data, LR annealing, long-context) | `schedules-and-annealing-literature` (2026-08-24 NBLM entry) | agent-supplied (NotebookLM pretraining notebook) | WSD, anneal-slice |  |
| (no ID) | Overtrained Language Models Are Harder to Fine-Tune — catastrophic overtraining; sensitivity to parameter transformations | `schedules-and-annealing-literature` (2026-08-24 NBLM entry) | agent-supplied (NotebookLM pretraining notebook) | plasticity, pretraining-to-posttraining |  |
| (no ID) | Power Lines — WD/BS scaling laws; AdamW timescale ~ power law of D/N; B_opt/B_crit scale with D only | `schedules-and-annealing-literature` (2026-08-24 NBLM entry) | agent-supplied (NotebookLM pretraining notebook) | WSD, TINY |  |
| (no ID) | Step Law (Predictable Scale Part I) — 3,700 runs; (LR,BS) landscape strictly convex; LR(N,D), BS(D) | `schedules-and-annealing-literature` (2026-08-24 NBLM entry) | agent-supplied (NotebookLM pretraining notebook) | WSD, TINY |  |
| (no ID) | Training Dynamics Impact Post-Training Quantization Robustness — PTQ error spikes at LR decay; WSD/LAWA/souping mitigate; refutes data-scale claim (Kumar et al. 2024) | `schedules-and-annealing-literature` (2026-08-24 NBLM entry) | agent-supplied (NotebookLM pretraining notebook) | WSD |  |
| (no ID) | Weight Decay Improves Language Model Plasticity — higher pretrain WD → better fine-tuning; attention pseudo-rank mechanism | `schedules-and-annealing-literature` (2026-08-24 NBLM entry) | agent-supplied (NotebookLM pretraining notebook) | plasticity, WSD |  |
| (no ID) | Causal Reasoning Favors Encoders — FOL, NL-vs-NNL ablation, depth stratification; encoder robustness vs decoder fragility | `generalization-and-ood-literature` (2026-08-24 NBLM entry) | agent-supplied (NotebookLM reasoning notebook) | — |  |
| (no ID) | Generalization or Hallucination? — OCR duality; factorized matrices + nuclear-norm implicit bias | `generalization-and-ood-literature` (2026-08-24 NBLM entry) | agent-supplied (NotebookLM reasoning notebook) | — |  |
| (no ID) | How and Why LLMs Generalize — five atomic cognitive skills; RL balanced vs SFT jagged over-specialization | `pretraining-to-posttraining` (2026-08-24 NBLM entry) | agent-supplied (NotebookLM reasoning notebook) | ELI, pretraining-to-posttraining |  |
| 2411.12580 | Procedural Knowledge in Pretraining Drives Reasoning — EK-FAC influence over 5M docs; code/math procedural docs drive reasoning | `pretraining-to-posttraining` (2026-08-24 NBLM entry) | agent-supplied (NotebookLM reasoning notebook) | DCARD, recipe relevance | ID Claude-added (inferred from title, plausibly Ruis et al.); notebook supplied none |
| (no ID) | From Word to World — LLMs as implicit text-based world models (Yixia Li); five environments; saturation vs entropy scaling | `world-models-literature` (2026-08-24 founding entry) | agent-supplied (NotebookLM world-models notebook) | minigrid staging |  |
| 2506.01622 | General agents contain world models (Richens) — no model-free shortcut; transition-function extraction from policies | `world-models-literature` (2026-08-24 founding entry) | agent-supplied (NotebookLM world-models notebook) | minigrid staging | ID Claude-added (inferred, plausibly the 2025 DeepMind paper); notebook supplied none |
| 2403.02054 | LLM-Based Evolutionary Optimizer: reasoning with elitism | `prompt-optimization-landscape` (2026-08-24 NBLM entry) | agent-supplied (NotebookLM LLM-optimization notebook) | TLC-opt |  |
| 2504.09247 | LLMs as Particle Swarm Optimizers | `prompt-optimization-landscape` (2026-08-24 NBLM entry) | agent-supplied (NotebookLM LLM-optimization notebook) | TLC-opt |  |
| 2511.00086 | Test-time compute-optimal scaling as an optimizable graph — Agent-REINFORCE textual gradients | `prompt-optimization-landscape` (2026-08-24 NBLM entry) | agent-supplied (NotebookLM LLM-optimization notebook) | TLC-opt |  |
| 2512.08609 | CogMCTS — cognitive-guided MCTS for heuristic evolution | `prompt-optimization-landscape` (2026-08-24 NBLM entry) | agent-supplied (NotebookLM LLM-optimization notebook) | TLC-opt |  |
| 2601.14446 | Diffusion LLMs for offline black-box optimization (dLLM) | `prompt-optimization-landscape` (2026-08-24 NBLM entry) | agent-supplied (NotebookLM LLM-optimization notebook) | TLC-opt |  |
| 2601.14523 | LLM-powered evolutionary code optimization on a phylogenetic tree | `prompt-optimization-landscape` (2026-08-24 NBLM entry) | agent-supplied (NotebookLM LLM-optimization notebook) | TLC-opt |  |
| 2601.14525 | Towards Execution-Grounded Automated AI Research — evolutionary search beats RL updates (48.0→69.4%; RL mode-collapses) | `prompt-optimization-landscape` (2026-08-24 NBLM entry) | agent-supplied (NotebookLM LLM-optimization notebook) | TLC-opt |  |
| (no ID) | Systematic survey: LLMs for evolutionary optimization (modeling → solving taxonomy) | `prompt-optimization-landscape` (2026-08-24 NBLM entry) | agent-supplied (NotebookLM LLM-optimization notebook) | TLC-opt |  |
| (no ID) | AutoML in the Age of LLMs — HPO/pipeline/NAS survey | `prompt-optimization-landscape` (2026-08-24 NBLM entry) | agent-supplied (NotebookLM LLM-optimization notebook) | — |  |
| (no ID) | Can LLMs Beat Classical HPO? (autoresearch) — TPE/CMA-ES beat pure LLMs; Centaur CMA-ES-state hybrid wins; state-tracking/density-estimation limitation | `prompt-optimization-landscape` (2026-08-24 NBLM entry) | agent-supplied (NotebookLM LLM-optimization notebook) | TLC-opt, whetstone context |  |
| (no ID) | Evolving Excellence / Artemis — semantically-aware mutation of agent workflows (+13.6% code, −36.9% tokens) | `prompt-optimization-landscape` (2026-08-24 NBLM entry) | agent-supplied (NotebookLM LLM-optimization notebook) | TLC-opt |  |
| (no ID) | Genetic Improvement for LLM-generated code (PSB2) — helps CodeLLaMA-7B, saturates at GPT-4 | `prompt-optimization-landscape` (2026-08-24 NBLM entry) | agent-supplied (NotebookLM LLM-optimization notebook) | TLC |  |
| (no ID) | JudgeFlow — block-level failure attribution for MCTS workflow optimization | `prompt-optimization-landscape` (2026-08-24 NBLM entry) | agent-supplied (NotebookLM LLM-optimization notebook) | TLC-opt |  |
| (no ID) | LLMize — OPRO-framework numerical/combinatorial optimization; LLM proposes annealing cooling rate | `prompt-optimization-landscape` (2026-08-24 NBLM entry) | agent-supplied (NotebookLM LLM-optimization notebook) | TLC-opt |  |
| (no ID) | Policy Improvement using Language Feedback Models — small offline LFMs distill LLM feedback for imitation (ALFWorld/ScienceWorld) | `prompt-optimization-landscape` (2026-08-24 NBLM entry) | agent-supplied (NotebookLM LLM-optimization notebook) | minigrid staging |  |
| (no ID) | ReCreate — meta-agent edits agent scaffolds from execution traces (>5% over human scaffolds) | `prompt-optimization-landscape` (2026-08-24 NBLM entry) | agent-supplied (NotebookLM LLM-optimization notebook) | TLC-opt |  |
| (no ID) | SCULPT — constraint-pruned MCTS for math reasoning (89.9% MATH on GPT-5.2) | `prompt-optimization-landscape` (2026-08-24 NBLM entry) | agent-supplied (NotebookLM LLM-optimization notebook) | — |  |
| (no ID) | Search over Self-Edit Strategies (SEAL variant) — archive helps; mode collapse needs novelty pressure | `prompt-optimization-landscape` (2026-08-24 NBLM entry) | agent-supplied (NotebookLM LLM-optimization notebook) | TLC-opt |  |
| (no ID) | Modeling Capabilities of LLMs for Sequential Decision Making — fail as direct policies, excel as reward models (NetHack/MetaWorld) | `prompt-optimization-landscape` (2026-08-24 NBLM entry) | agent-supplied (NotebookLM LLM-optimization notebook) | minigrid staging |  |
| (no ID) | Survey: LLMs for Automated Planning — heuristic guides, not standalone planners | `prompt-optimization-landscape` (2026-08-24 NBLM entry) | agent-supplied (NotebookLM LLM-optimization notebook) | minigrid staging |  |
| (no ID) | Evolutionary prompt optimization discovers emergent multimodal reasoning strategies (GPT-4o-mini, 20 examples) | `prompt-optimization-landscape` (2026-08-24 NBLM entry) | agent-supplied (NotebookLM LLM-optimization notebook) | TLC-opt |  |
| (no ID) | HAPO — hierarchical attribution prompt optimization; UCB selection; +13.28% over zero-shot CoT at fewer API calls than OPRO/APE | `prompt-optimization-landscape` (2026-08-24 NBLM entry) | agent-supplied (NotebookLM LLM-optimization notebook) | TLC-opt, C2 budget framing |  |
| (no ID) | Hayou et al. — Optimal Embedding Learning Rate in LLMs (LVP, √d-rule; µP breaks in large-vocab regime) | `parametrization-and-hp-transfer` (2026-08-24 NBLM entry) | agent-supplied (NotebookLM tokenization notebook) | ERD (embedding-reset), TINY, WSD |  |
| (no ID) | Kiyono et al. — BPE-dropout bridging subword/character segmentation in one pretrained model | `reinit-and-transfer-literature` (2026-08-24 entry) | agent-supplied (NotebookLM tokenization notebook) | ERD context |  |
| (no ID) | Singh et al. — Cross-Tokenizer LLM Distillation through a Byte-Level Interface (BLD; IFEval collapse) | `reinit-and-transfer-literature` (2026-08-24 entry) | agent-supplied (NotebookLM tokenization notebook) | ERD / interface transfer |  |
| 1508.07909 | Sennrich et al. — NMT of Rare Words with Subword Units (BPE) | `reinit-and-transfer-literature` (2026-08-24 entry) | agent-supplied (NotebookLM tokenization notebook) | canon | ID Claude-added (inferred, canonical); notebook supplied none |
| (no ID) | Dang — Tokenization and Morphology in mT5 vs ByT5 (probing; layer-depth and irregularity×data-share results) | `reinit-and-transfer-literature` (2026-08-24 entry) | agent-supplied (NotebookLM tokenization notebook) | — |  |
| (no ID) | Zhang et al. — VocabTailor: dynamic vocabulary selection for SLMs (99% vocab-memory compression) | `reinit-and-transfer-literature` (2026-08-24 entry) | agent-supplied (NotebookLM tokenization notebook) | — |  |
| 2010.11934 | Xue et al. — mT5 (mC4; DPT vs accidental translation) | `reinit-and-transfer-literature` (2026-08-24 entry) | agent-supplied (NotebookLM tokenization notebook) | canon | ID Claude-added (inferred, canonical); notebook supplied none |
| (no ID) | Levis — TinyOS networking abstractions history (off-topic source in the notebook; systems-design analogies only) | bundle header note | agent-supplied (NotebookLM tokenization notebook) | — | non-ML source |
| (no ID) | PromptCS — continuous prompt agent (BiLSTM+MLP) over frozen LLMs for code summarization | `prompt-optimization-landscape` + `code-compression-literature` (2026-08-24 NBLM entries) | agent-supplied (NotebookLM AI4SE notebook) | TLC (soft-prompt contrast) |  |
| (no ID) | Fluorine — C/Go→Rust translation with cross-language differential fuzzing; restart beats counterexample repair 7–21%; ~100 LoC threshold | `prompt-optimization-landscape` + `code-compression-literature` (2026-08-24 NBLM entries) | agent-supplied (NotebookLM AI4SE notebook) | TLC-opt (loop design) |  |
| (no ID) | Source Code Summarization in the Era of LLMs (empirical study) — BLEU/METEOR/BERTScore anti-correlate with human ratings; GPT-4-as-judge closest | `prompt-optimization-landscape` + `code-compression-literature` (2026-08-24 NBLM entries) | agent-supplied (NotebookLM AI4SE notebook) | TLC (d-quality metrics) |  |
...
| (no ID) | RefactorBench — 100 multi-file Python refactoring tasks; agents 22% vs humans 87%; past-state conditioning +43.9% | `code-benchmarks-landscape` (2026-08-24 NBLM refactoring entry) | agent-supplied (NotebookLM refactoring notebook) | agentic-dev context |  |
| (no ID) | CodeTaste — human-alignment refactoring benchmark; instructed 69.6% vs open-track single digits | `code-benchmarks-landscape` (2026-08-24 NBLM refactoring entry) | agent-supplied (NotebookLM refactoring notebook) | agentic-dev context |  |
| (no ID) | MANTRA — RAG + Developer/Reviewer/Repair agents for method-level refactoring; Reviewer agent largest contributor | `code-benchmarks-landscape` (2026-08-24 NBLM refactoring entry) | agent-supplied (NotebookLM refactoring notebook) | agentic-dev context |  |
| (no ID) | Agentic Refactoring empirical study (AIDev; 15,451 refactorings) — janitorial skew; 53.9% tangled commits | `code-benchmarks-landscape` (2026-08-24 NBLM refactoring entry) | agent-supplied (NotebookLM refactoring notebook) | agentic-dev context |  |
| (no ID) | Where Do AI Coding Agents Fail? — 33k agentic PRs; rejection taxonomy | `code-benchmarks-landscape` (2026-08-24 NBLM refactoring entry) | agent-supplied (NotebookLM refactoring notebook) | agentic-dev context |  |
| (no ID) | Do AI Agents Really Improve Code Readability? — readability commits add volume, reduce maintainability indices | `code-benchmarks-landscape` (2026-08-24 NBLM refactoring entry) | agent-supplied (NotebookLM refactoring notebook) | agentic-dev context |  |
| (no ID) | Beyond Strict Rules — LLM vs static-tool code-smell detection; majority-voting hybrid | `code-benchmarks-landscape` (2026-08-24 NBLM refactoring entry) | agent-supplied (NotebookLM refactoring notebook) | — |  |
| (no ID) | Dagstuhl 24431 — automated programming & program repair seminar report (context-degradation, energy trade-off, trust problem) | `code-benchmarks-landscape` (2026-08-24 NBLM refactoring entry) | agent-supplied (NotebookLM refactoring notebook) | — |  |
| 2603.23443 | Evaluating LLM-Based Test Generation Under Software Evolution (metadata only) | `code-benchmarks-landscape` (2026-08-24 NBLM refactoring entry) | agent-supplied (NotebookLM refactoring notebook) | — |  |
| (no ID) | DNO — Direct Nash Optimization (general-preference alignment; Nash equilibrium) | `pretraining-to-posttraining` (2026-08-24 self-improvement entry) | agent-supplied (NotebookLM refactoring notebook) | — |  |
| (no ID) | Iterative DPO empirical study — DPO-VP matches RL reasoning at low compute; amplifies existing patterns | `pretraining-to-posttraining` (2026-08-24 self-improvement entry) | agent-supplied (NotebookLM refactoring notebook) | ELI context |  |
| (no ID) | SCPO — self-consistency preference optimization (+22.7pp GSM8K unsupervised) | `pretraining-to-posttraining` (2026-08-24 self-improvement entry) | agent-supplied (NotebookLM refactoring notebook) | — |  |
| (no ID) | Self-Rewarding Language Models (LLM-as-own-judge DPO loop) | `pretraining-to-posttraining` (2026-08-24 self-improvement entry) | agent-supplied (NotebookLM refactoring notebook) | — |  |
| (no ID) | Why Self-Rewarding Works — theory: initialization-dependent single steps, iteration neutralizes | `pretraining-to-posttraining` (2026-08-24 self-improvement entry) | agent-supplied (NotebookLM refactoring notebook) | — |  |
| 2508.06026 | Temporal Self-Rewarding LMs (metadata only) | `pretraining-to-posttraining` (2026-08-24 self-improvement entry) | agent-supplied (NotebookLM refactoring notebook) | — |  |
| 2501.00663 | Titans — learning to memorize at test time (metadata only) | `pretraining-to-posttraining` (2026-08-24 self-improvement entry) | agent-supplied (NotebookLM refactoring notebook) | — |  |
| (no ID) | Self-Improvement of LLMs survey — lifecycle taxonomy; data autophagy warning | `pretraining-to-posttraining` (2026-08-24 self-improvement entry) | agent-supplied (NotebookLM refactoring notebook) | — |  |
| (no ID) | Pluralistic Alignment survey — Overton/steerable/distributional modes | `pretraining-to-posttraining` (2026-08-24 self-improvement entry) | agent-supplied (NotebookLM refactoring notebook) | — |  |
| (no ID) | LLM Post-Training: Data Synthesis and Algorithms — Nectar, Meta-Rewarding, P3O, TPO | `pretraining-to-posttraining` (2026-08-24 self-improvement entry) | agent-supplied (NotebookLM refactoring notebook) | — |  |
| (no ID) | Learning from Rewards in Text Generation — GOLD, IRPO | `pretraining-to-posttraining` (2026-08-24 self-improvement entry) | agent-supplied (NotebookLM refactoring notebook) | — |  |
| 2411.04330 | Scaling Laws for Precision — quantization degradation increases with over-training; 7–8-bit compute-optimal | `schedules-and-annealing-literature` (2026-08-24 precision entry) | agent-supplied (NotebookLM refactoring notebook) | WSD (PTQ dispute) | ID Claude-added (inferred); the 'Kumar et al. 2024' the PTQ-robustness paper disputes — read together at verification |
| (no ID) | Jacobs, Jordan, Nowlan & Hinton 1991 — Adaptive Mixtures of Local Experts (pre-arXiv) | `moe-literature` (2026-08-24 NBLM entry) | agent-supplied (NotebookLM MoE notebook) | MOVE/PART/MSUITE |  |
| 2101.03961 | Switch Transformers — Top-1 routing, selective precision, 0.1× init | `moe-literature` (2026-08-24 NBLM entry) | agent-supplied (NotebookLM MoE notebook) | MOVE/PART/MSUITE | ID Claude-added (canonical); notebook supplied none |
| 2202.09368 | Expert Choice Routing — experts pick tokens; perfect balance | `moe-literature` (2026-08-24 NBLM entry) | agent-supplied (NotebookLM MoE notebook) | MOVE/PART/MSUITE | ID Claude-added (canonical) |
| 2103.16716 | BASE Layers — routing as linear assignment | `moe-literature` (2026-08-24 NBLM entry) | agent-supplied (NotebookLM MoE notebook) | MOVE/PART/MSUITE | ID Claude-added (canonical) |
| 2202.08906 | ST-MoE — router z-loss; sparse fine-tuning protocols differ | `moe-literature` (2026-08-24 NBLM entry) | agent-supplied (NotebookLM MoE notebook) | MOVE/PART/MSUITE, pipeline comparability | ID Claude-added (canonical) |
| 2211.15841 | MegaBlocks — block-sparse dropless MoE kernels | `moe-literature` (2026-08-24 NBLM entry) | agent-supplied (NotebookLM MoE notebook) | MSUITE | ID Claude-added (canonical) |
| 2412.19437 | DeepSeek-V3 — aux-loss-free balancing, MLA, MTP, FP8/DualPipe (dedicated MoE row; fragment rows exist) | `moe-literature` (2026-08-24 NBLM entry) | agent-supplied (NotebookLM MoE notebook) | MOVE/PART/MSUITE | ID previously on record via MTP context |
| 2409.02060 | OLMoE — 64/8 dropless; from-scratch specialization vs upcycled Mixtral redundancy | `moe-literature` (2026-08-24 NBLM entry) | agent-supplied (NotebookLM MoE notebook) | MOVE/PART/MSUITE | ID Claude-added (canonical) |
| (no ID) | DeepSpeed-MoE — PR-MoE pyramid expert placement; Mixture-of-Students distillation | `moe-literature` (2026-08-24 NBLM entry) | agent-supplied (NotebookLM MoE notebook) | MSUITE |  |
| (no ID) | HMoE — heterogeneous expert sizes + P-Penalty loss; hard tokens → large experts | `moe-literature` (2026-08-24 NBLM entry) | agent-supplied (NotebookLM MoE notebook) | MOVE/PART |  |
| (no ID) | Holistic Scaling Laws for Optimal MoE Architecture — near-optimal shape band widens with compute | `moe-literature` (2026-08-24 NBLM entry) | agent-supplied (NotebookLM MoE notebook) | MSUITE, TINY (proxy-shape caution) |  |
| (no ID) | Efficiency Leverage (Towards Greater Leverage) — activation ratio as primary efficiency driver; 300+ models | `moe-literature` (2026-08-24 NBLM entry) | agent-supplied (NotebookLM MoE notebook) | MSUITE |  |
| 2202.01169 | Unified Scaling Laws for Routed Language Models (Clark et al.) — S-BASE scales best | `moe-literature` (2026-08-24 NBLM entry) | agent-supplied (NotebookLM MoE notebook) | MSUITE | ID Claude-added (canonical) |
| (no ID) | Towards a Comprehensive Scaling Law of MoE — G_opt≈6.78, S_opt≈0.31 (Dolma, WSD) | `moe-literature` (2026-08-24 NBLM entry) | agent-supplied (NotebookLM MoE notebook) | MSUITE, WSD |  |
| (no ID) | Scaling Laws Across Architectures (dense vs MoE) — MoE as implicit regularizer; LR/BS rules transfer | `moe-literature` (2026-08-24 NBLM entry) | agent-supplied (NotebookLM MoE notebook) | MSUITE, matched-loss pillar |  |
| (no ID) | HDEE — heterogeneous domain expert ensembles; size heterogeneity beats step heterogeneity | `moe-literature` (2026-08-24 NBLM entry) | agent-supplied (NotebookLM MoE notebook) | MOVE |  |
| (no ID) | Joint MoE Scaling Laws (memory-aware) — MoE can be model-memory optimal; LR-vs-expert-count scaling | `moe-literature` (2026-08-24 NBLM entry) | agent-supplied (NotebookLM MoE notebook) | MSUITE |  |
| (no ID) | Parameters vs FLOPs — optimal sparsity grows with budget; reading comprehension favors density | `moe-literature` (2026-08-24 NBLM entry) | agent-supplied (NotebookLM MoE notebook) | MSUITE |  |
| 2402.07871 | Scaling Laws for Fine-Grained MoE (Krajewski et al.) — optimal granularity beats dense at any budget | `moe-literature` (2026-08-24 NBLM entry) | agent-supplied (NotebookLM MoE notebook) | MSUITE | ID Claude-added (canonical) |
| (no ID) | Capacity-Aware Inference — straggler effect; test-time load 7× average; token/expanded drop | `moe-literature` (2026-08-24 NBLM entry) | agent-supplied (NotebookLM MoE notebook) | MOVE (routing observable) |  |
| (no ID) | Optimal Sparsity of MoE for Reasoning (B2) — sparsity hurts reasoning at scale; unrecoverable by GRPO/TTC | `moe-literature` (2026-08-24 NBLM entry) | agent-supplied (NotebookLM MoE notebook) | MSUITE, pretraining-to-posttraining | convergent with Mixture of Parrots |
| (no ID) | Scaling Fine-Grained MoE Beyond 50B (B3) — granularity gains persist; softmax/Top-k order sensitivity | `moe-literature` (2026-08-24 NBLM entry) | agent-supplied (NotebookLM MoE notebook) | MSUITE |  |
| (no ID) | A4 survey — expert representation collapse >99% similarity without regularization; OMoE/MoDE | `moe-literature` (2026-08-24 NBLM entry) | agent-supplied (NotebookLM MoE notebook) | MOVE/PART |  |
| (no ID) | A1/A2 MoE surveys — taxonomies; FFN param share ~90%, activation sparsity asymmetry; DS-MoE; upcycling | `moe-literature` (2026-08-24 NBLM entry) | agent-supplied (NotebookLM MoE notebook) | MOVE/PART/MSUITE |  |
