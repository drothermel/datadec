...
| [moe-recipe-suite.md](moe-recipe-suite.md) | Whether to run a data-varied, architecture-fixed MoE suite on the Slicing-and-Dicing repo (a project candidate), and separately whether the two "why" analyses go into that paper's rebuttal; the earlier "reweighting" response it references is not yet captured. Sweep final checkpoints confirmed available (HF upload pending) |
