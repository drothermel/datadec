...
| [cnn-deconstruction-ladder.md](staging/cnn-deconstruction-ladder.md) | Ablation ladder over CNN building blocks (skips, BN, activation, init, optimizer, schedule, augmentation) back to LeNet, cheap CIFAR-10/ImageNet-subset runs with seeds + sweeps and landscape metrics; the `deconCNN` project of ~2025; a first 15-config × 3–6-seed ablation dataset existed by 2025-06-12 (not on file). Gate: Danielle says whether it is still live, and locates the dataset — promote to a project doc or archive as a past learning project |
| [tiny-model-specialization.md](staging/tiny-model-specialization.md) | Hypothesis: ~10M models can be fit to narrow tasks ("the 10M pandas specialist") by optimizing only an outer layer — an external, large-model-fit rewriter / DSL interface — without weight updates; breakdown by task specificity, data, external-vs-PEFT, and generality loss. Reframed in-conversation as elicitation-as-instrument: a cliff curve of wrapper-only extractable success vs. DataDecide size at fixed optimizer + budget, and whether the loop reveals pre/post-training movement (ΔS, stability, iterations-to-threshold) where zero-shot is flat; controls: outer-model-only, shuffled wrapper. TLC machinery with a tiny frozen executor; first experiment is a capability existence test under an oracle interface. Gate: Danielle picks experiment shape + eval; prior-art pass (prompt tuning at small scale is a known headwind; TinyStories / phi-1 / DSL lines) |
