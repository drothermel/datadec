...
| [convergent-intake-findings.md](reference/convergent-intake-findings.md) | Cross-cutting threads recurring across independent intake sources (dated synthesis passes): amplify-don't-instill/mode collapse in cheap improvement loops; over-training's four downstream costs; representation surface changes frozen-model capability. Pointers to the accumulator entries holding detail; convergence ≠ verification |
