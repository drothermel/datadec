...
def show_available_dataframes(dd: DataDecide) -> None:
    ...
    categories = {
        ...
        "Intermediate Processing": [
            "dwn_metrics_expanded", "dwn_mmlu_averaged", "dwn_pivoted", 
            "ppl_dwn_merged", "step_to_token_compute"
        ],
        "Parsed Data": ["ppl_parsed", "dwn_parsed"],
        "Final Datasets": ["full_eval", "mean_eval", "std_eval"],
        "Analysis-Ready": [
            "full_eval_with_details", "full_eval_with_lr",
            "mean_eval_with_details", "mean_eval_with_lr"
        ]
    }
        ...
        for df_name in dataframes:
            if df_name in dd.paths.available_dataframes:
                try:
                    ...
                    print(f"   • {df_name:<25} {df.shape[0]:>8,} rows × {df.shape[1]:>3} cols")
...
def demo_basic_access(dd: DataDecide) -> None:
    ...
    print(f"   • Full evaluation data:  {dd.full_eval.shape[0]:>8,} rows × {dd.full_eval.shape[1]:>3} cols")
    print(f"   • Mean evaluation data:  {dd.mean_eval.shape[0]:>8,} rows × {dd.mean_eval.shape[1]:>3} cols") 
    print(f"   • Model details:         {dd.model_details.shape[0]:>8,} rows × {dd.model_details.shape[1]:>3} cols")
    print(f"   • Dataset details:       {dd.dataset_details.shape[0]:>8,} rows × {dd.dataset_details.shape[1]:>3} cols")
...
def demo_analysis_features(dd: DataDecide, min_params: str = "10M") -> None:
    ...
    print(f"   Final shape: {analysis_df.shape[0]:,} rows × {analysis_df.shape[1]} cols")
    ...
    print("   • Core columns:", ", ".join(col for col in key_cols if col in analysis_df.columns))
    if lr_cols:
        print("   • Learning rate columns:", ", ".join(lr_cols[:3]) + ("..." if len(lr_cols) > 3 else ""))
...
def demo_data_exploration(dd: DataDecide) -> None:
    ...
    model_sizes = sorted(consts.MODEL_SHAPES.keys(), key=lambda x: consts.HARDCODED_SIZE_MAPPING[x])
    for size in model_sizes:
        params = consts.HARDCODED_SIZE_MAPPING[size]
        config = consts.MODEL_SHAPES[size]
        print(f"   • {size:<6} ({params:>11,} params) - {config['n_layers']} layers, {config['d_model']} dim")
...
def demo_filtering_options(dd: DataDecide, model_size: Optional[str] = None, 
                          data_recipe: Optional[str] = None) -> None:
...
Examples:
  ...
  python download_data.py --model_size 1B --data_recipe "Dolma1.7"  # Filtered demo
    ...
    print(f"🚀 DataDecide Setup and Demo")
    ...
    dd = DataDecide(
        ...
        recompute_from=args.recompute_from,
        verbose=True
    )
    ...
    print(f"   • Quick analysis: dd.get_analysis_df()")
