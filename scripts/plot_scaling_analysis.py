...
def generate_config1_plot(df, test_params, test_data, plots_dir):
    ...
    try:
        ...
        with FigureManager(
            ...
            fm.fig.suptitle("Config 1: Params as lines, data as subplots (Native dr_plotter)", fontsize=16)
            ...
            for i, data_val in enumerate(test_data):
                ...
                if not subset.empty:
                    fm.plot(
                        "line", 0, i, subset,
                        x="tokens", y="pile-valppl", hue_by="params",
                        title=f"Data: {data_val}", linewidth=2, alpha=0.9,
                    )
        
        fig1_path = plots_dir / "config1_params_lines_data_subplots.png"
        fm.fig.savefig(fig1_path, dpi=150, bbox_inches="tight")
        print(f"✓ Saved config1_params_lines_data_subplots.png")
        return True
...
def generate_config2_plot(df, test_params, test_data, plots_dir):
    ...
    try:
        ...
        with FigureManager(
            ...
            fm.fig.suptitle("Config 2: Data as lines, params as subplots (Native dr_plotter)", fontsize=16)
            ...
            for i, param_val in enumerate(test_params):
                ...
                if not subset.empty:
                    fm.plot(
                        "line", 0, i, subset,
                        x="tokens", y="pile-valppl", hue_by="data",
                        title=f"Params: {param_val}", linewidth=2, alpha=0.9,
                    )
        
        fig2_path = plots_dir / "config2_data_lines_params_subplots.png"
        fm.fig.savefig(fig2_path, dpi=150, bbox_inches="tight")
        print(f"✓ Saved config2_data_lines_params_subplots.png")
        return True
...
def generate_config4_plot(df, test_params, test_data, plots_dir):
    ...
    try:
        ...
        df_config4 = df[
            (df['params'].isin(config4_params)) & 
            (df['data'].isin(config4_data))
        ].copy()
        ...
        with FigureManager(
            ...
            fm.fig.suptitle("Config 4: Multi-Metric with Grouped Legends (Native dr_plotter)", fontsize=14)
            
            # First subplot: pile-valppl
            fm.plot(
                "line", 0, 0, df_config4,
                x="tokens", y="pile-valppl", 
                hue_by="data", style_by="params",
                title="Pile Validation Perplexity", linewidth=2, alpha=0.8,
            )
            
            # Second subplot: mmlu_average_correct_prob
            fm.plot(
                "line", 0, 1, df_config4,
                x="tokens", y="mmlu_average_correct_prob",
                hue_by="data", style_by="params",
                title="MMLU Average Accuracy", linewidth=2, alpha=0.8,
            )
        
        fig4_path = plots_dir / "config4_multi_metric.png"
        fm.fig.savefig(fig4_path, dpi=150, bbox_inches="tight")
        print(f"✓ Saved config4_multi_metric.png")
        return True
...
def generate_config5_plot(df, test_params, test_data, plots_dir):
    ...
    try:
        ...
        with FigureManager(
            ...
            fm.fig.suptitle("Config 5: Single data recipe, more params (Native dr_plotter)", fontsize=16)
            
            fm.plot(
                "line", 0, 0, df_config5,
                x="tokens", y="pile-valppl", hue_by="params",
                title=f"Data: {single_data[0]}", linewidth=2, alpha=0.9,
            )
        
        fig5_path = plots_dir / "config5_single_data_more_params.png"
        fm.fig.savefig(fig5_path, dpi=150, bbox_inches="tight")
        print(f"✓ Saved config5_single_data_more_params.png")
        return True
...
def generate_config6_plot(df, test_params, test_data, plots_dir):
    ...
    try:
        ...
        with FigureManager(
            ...
            fm.fig.suptitle("Config 6: Stacked params as lines (Native dr_plotter)", fontsize=16)
            ...
            for metric_idx, (metric, metric_title) in enumerate(zip(metrics, metric_titles)):
                for data_idx, data_val in enumerate(test_data):
                    ...
                    if not subset.empty:
                        fm.plot(
                            "line", metric_idx, data_idx, subset,
                            x="tokens", y=metric, hue_by="params",
                            title=f"{metric_title}\nData: {data_val}" if metric_idx == 0 else f"Data: {data_val}",
                            linewidth=2, alpha=0.9,
                        )
        
        fig6_path = plots_dir / "config6_stacked_params_lines.png"
        fm.fig.savefig(fig6_path, dpi=150, bbox_inches="tight")
        print(f"✓ Saved config6_stacked_params_lines.png")
        return True
...
def generate_config7_plot(df, test_params, test_data, plots_dir):
    ...
    try:
        ...
        with FigureManager(
            rows=2, cols=len(config7_params),
            ...
            fm.fig.suptitle("Config 7: Stacked data as lines (Native dr_plotter)", fontsize=16)
            ...
            for metric_idx, (metric, metric_title) in enumerate(zip(metrics, metric_titles)):
                for param_idx, param_val in enumerate(config7_params):
                    subset = df[
                        (df['params'] == param_val) & 
                        (df['data'].isin(config7_data))
                    ].copy()
                    if not subset.empty:
                        fm.plot(
                            "line", metric_idx, param_idx, subset,
                            x="tokens", y=metric, hue_by="data",
                            title=f"{metric_title}\nParams: {param_val}" if metric_idx == 0 else f"Params: {param_val}",
                            linewidth=2, alpha=0.9,
                        )
        
        fig7_path = plots_dir / "config7_stacked_data_lines.png"
        fm.fig.savefig(fig7_path, dpi=150, bbox_inches="tight")
        print(f"✓ Saved config7_stacked_data_lines.png")
        return True
...
def generate_all_plots(df, test_params, test_data, plots_dir):
    ...
    results.append(("Config 1", generate_config1_plot(df, test_params, test_data, plots_dir)))
    results.append(("Config 2", generate_config2_plot(df, test_params, test_data, plots_dir))) 
    results.append(("Config 3", generate_config3_plot(df, test_params, test_data, plots_dir)))
    results.append(("Config 4", generate_config4_plot(df, test_params, test_data, plots_dir)))
    results.append(("Config 5", generate_config5_plot(df, test_params, test_data, plots_dir)))
    results.append(("Config 6", generate_config6_plot(df, test_params, test_data, plots_dir)))
    results.append(("Config 7", generate_config7_plot(df, test_params, test_data, plots_dir)))
...
def main():
    ...
    print("Benefits: 40-92% code reduction, enhanced reliability, zero custom functions")
